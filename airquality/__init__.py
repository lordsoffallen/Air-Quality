...
from airquality import luftdaten, utils

__version__ = "0.1"


def compare_sensor_data(sensors, columns, start_date, end_date,
    ...
    Args:
        sensors: sequence of sensor objects, instances of
            utils.BaseSensor
        columns: sequence of column names of the data to use; order
    ...
    for sensor, column in zip(sensors, columns):
...
"""
def compare_nearest_luftdaten_sensors(sensor, start_date, end_date, **retrieval_kwargs):
    ""Compare a sensor's measurements (hourly means) to those of the
    closest LUFTDATEN sensor(s) that measure equivalent phenomena.
    ...
    # Get the nearest sensors within 10km
    nearest_luftdaten_sensors = (luftdaten.search_proximity(radius=10))
    nearest_luftdaten_sensor = nearest_luftdaten_sensors[0]
    combined_data_pieces = []
    plots = []
    
    for phenomenon in nearest_luftdaten_sensor:
        luftdaten_time_series_id = nearest_luftdaten_sensor.at["time series id", phenomenon]
        luftdaten_phenomenon = nearest_luftdaten_sensor.at["phenomenon", phenomenon]
        distance = nearest_luftdaten_sensor.at["distance", phenomenon]
        luftdaten_sensor = luftdaten.Sensor(luftdaten_time_series_id)
        luftdaten_station_label = nearest_luftdaten_sensors.at["station_label", phenomenon]
        (combined_data_piece,
         plot) = compare_sensor_data([sensor, luftdaten_sensor],
                                     [phenomenon, luftdaten_phenomenon],
