...
from airquality import utils
from airquality.utils import _cache_dir, retrieve, BaseSensor
...
class Sensor(BaseSensor):
    ...
    def get_metadata(self, **retrieval_kwargs):
        ...
        try:
            metadata = (parsed.drop(columns=["sensordatavalues", "timestamp"]).iloc[0])
            ...
            if {"P1", "P2"}.issubset(current["value_type"]):
                current = current.replace({"P1": "PM10", "P2": "PM2.5"}).set_index("value_type")["value"]

                # Convert argument to numeric type
                current = (pd.to_numeric(current).replace([999.9, 1999.9], pd.np.nan))
            else:
    ...
    def get_measurements(self, start_date, end_date, **retrieval_kwargs):
        ...
        if self.sensor_type is None:
            self.sensor_type = input("Type of sensor {} has not been set yet. "
                                     "Enter sensor type: ".format(self.sensor_id))
        ...
        for date in pd.date_range(start_date, end_date):
            ...
            data = retrieve(cache_file=filepath,
                            ...
                            label=("luftdaten.info data for sensor {} on {}"
                                   .format(self.sensor_id, date_iso)),
            ...
            # TODO Add more sensor parsing method especially for ppd42ns

            if self.sensor_type in ("SDS011", "HPM", "PMS3003", "PMS5003", "PMS7003"):
                # Rename the original column names
                data = (data[["P1", "P2"]]
                        .rename(columns={"P1": "PM10", "P2": "PM2.5"}))
            elif self.sensor_type in ("DHT22", "BME280", "BMP180", "BMP280", "HTU21D"):
        ...
        # Check if data contains particle values.
        if {"PM10", "PM2.5"}.issubset(self.measurements.columns):
            self.clean_measurements_with_particles()

    def clean_measurements_with_particles(self):
        """Remove invalid measurements.

        This function only cleans data with PM10 and PM2.5 values.
...
def search_proximity(lat=50.848, lon=4.351, radius=8):
    ...
    # Set Dataframe index to sensor id
...
def evaluate_near_sensors(start_date, end_date, lat=48.779, lon=9.179,
    ...
    Coordinates and radius default to Stuttgart.
